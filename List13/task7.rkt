; based on state-mc.rkt
; Changed marked with #CHANGE
  ...
  (cond
    ...
    [(s-exp-match? `{begin ANY ANY} s)
     (beginE(parse (second (s-exp->list s)))
            (parse (third (s-exp->list s))))]
    ; #CHANGE add matching for whileE
...
(define (eval [e : Exp] [env : Env]) : Value
    (type-case Exp e
        ...
        (voidV))]
        ; #CHANGE add whileE to eval function
        ; def in a outside function for easier rec
        [(whileE c e)
...
; #CHANGE add eval-while function
(define (eval-while [c : Exp] [e : Exp] [result : Value] [env : Env]) : Value
    (let ([cond-val (boolV-b (eval c env))])
        (if cond-val
