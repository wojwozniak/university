...
(define (desugar [e : Exp]) : Exp
    ; We are only desugaring condE, so we can just return the expression if it is not a condE
    (type-case Exp e
      [(condE cs) (desugar-cond cs)]
      [else e]
    )
  )

; desugar-cond function takes a list of (condition, expression) pairs and returns desugared expression.
(define (desugar-cond [cs : (Listof (Exp * Exp))]) : Exp
  ; If the list is empty, return default value
  (type-case (Listof (Exp * Exp)) cs
    [empty (numE 42)]
    ; Otherwise we convert the first pair to an ifE and recursively call desugar-cond on the rest of the list
    [(cons c cs) (ifE (fst c) (snd c) (desugar-cond cs))]
  )
)

; We also need to modify run function to desugar the expression before evaluating it
